<p align="center">
<img src="https://link.jabbar.id/cover.jpg" alt="Cover Image"/>
</p>

# Hi there, ジャッバルと申します 👋

I’m **Jabbar Ali Panggabean**, a full-stack engineer building systems that *work*, *scale*, and *deliver with quiet confidence*.
I favour clarity, reliability, and technology that actually delivers.

## About Me

* **Focus**: Full-stack · DevOps · AI/IoT
* **Tech I enjoy**: TypeScript · Golang · Python · React · Next.js · Docker · AWS
* **Languages**: English · Bahasa Indonesia · 日本語 (N4)
* **Location**: Somewhere between Jakarta & Bandung (and a stable internet connection)
* **Technical Background**: Experienced with backend frameworks like Golang, Python, Node.js, and Laravel; frontend stacks such as React, Next.js, Tailwind, and Vite; and infrastructure tools like AWS, Docker, CI/CD, Redis, and NATS. Also familiar with YOLOv8, OpenCV, Unity, MQTT, and BLE.
<br>
<img src="https://skillicons.dev/icons?i=ts,react,nextjs,golang,python,aws,docker,unity,vite&perline=10" alt="Tech Icons"/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=jabbar-app&bg_color=0d1117&color=58a6ff&line=58a6ff&point=ffffff&area=true&hide_border=true" alt="Activity Graph"/>

## Links

* **Portfolio** → [link.jabbar.id](https://link.jabbar.id/)
* **Email** → [box@jabbar.id](mailto:box@jabbar.id)
* **LinkedIn Profile** → [linkedin.com/in/jabbarpanggabean](https://www.linkedin.com/in/jabbarpanggabean/)
* **Medium Articles** → [medium.com/@jabbarpanggabean](https://medium.com/@jabbarpanggabean)
* **Inisiator Page** → [inisiator.com/@jabbarp_](https://inisiator.com/@jabbarp_)

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:2b5876,100:4e4376&height=120&section=footer&text=よろしくお願いします！&fontAlign=50&fontAlignY=75&fontColor=ffffff" alt="footer"/>
</p>

